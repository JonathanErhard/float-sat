...
// Simulate a basic loop where we read data from the sensors and apply the Kalman filter
	...
	Adcs::accelData.x=imu.accelerometer[0]; // Accelerometer data (example)
	Adcs::accelData.y=imu.accelerometer[1]  ;  // Accelerometer data (example)
	Adcs::accelData.z=imu.accelerometer[2]  ;  // Accelerometer data (example)

	Adcs::gyroData.x= imu.gyroscope[0];  // Gyroscope data (example)
	Adcs::gyroData.y= imu.gyroscope[1];  // Gyroscope data (example)
	Adcs::gyroData.z= imu.gyroscope[2];  // Gyroscope data (example)

	Adcs::magData.x= imu.magnetometer[0];  // Magnetometer data (example)
	Adcs::magData.y= imu.magnetometer[1];  // Magnetometer data (example)
	Adcs::magData.z= imu.magnetometer[2]; 
	...
	generated::attitudeDeterminationTopic.publish({state.x,state.y});
...
bool Adcs::handleTelecommandSetControlMode(const generated::SetControlMode &setControlMode) {
	RODOS::PRINTF("ADCS:setControlMode\n");
	return false;
}
